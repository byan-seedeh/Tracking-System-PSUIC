 ...
 */
const CustomSelect = ({ options = [], value, onChange, placeholder, disabled = false, className = "" }) => {
    ...
    return (
        <div className={`relative w-full ${disabled ? "opacity-60 pointer-events-none" : ""}`} ref={dropdownRef}>
            ...
            <div
                ...
                className={`
          w-full px-4 h-full border flex items-center justify-between cursor-pointer transition-all
          ${className ? className : "bg-white border-[#E3E8EF] rounded-xl"} 
          ${isOpen ? "border-[#35A8E0] shadow-[0_0_0_2px_rgba(53,168,224,0.15)]" : ""}
          ${!className && !isOpen ? "hover:border-blue-300" : ""}
        `}
            >
                <span className={`font-normal ${selectedItem ? "text-slate-700" : "text-[#A0AEC0]"}`}>
