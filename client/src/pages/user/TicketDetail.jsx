    ...
    <div className="min-h-screen bg-gray-50 pb-20 font-sans text-gray-900">
      ...
      <div className="max-w-4xl mx-auto px-6 mt-6 pb-6 space-y-6 animate-in fade-in duration-500 relative z-10">
        ...
        <div className="bg-white rounded-xl shadow-[0_2px_8px_rgba(0,0,0,0.05)] p-6 space-y-4">
          {/* Row item helper */}
          {[
            { label: "Ticket ID", value: "#" + ticket?.id, color: "text-blue-600 font-semibold" },
            ...
            { label: "Room", value: ticket?.room?.roomNumber || "-" },
            {
              ...
              value: (
                <span
                  className={`px-2.5 py-1 rounded-lg text-xs font-medium ${ticket?.urgency === "Critical" || ticket?.urgency === "High"
            ...
            <div key={idx} className="flex justify-between items-center">
              ...
              {item.isBadge ? (
                ...
                <span className={`text-right font-medium text-base ${item.color || "text-gray-900"}`}>{item.value}</span>
              )}
            </div>
          ))}
        </div>
        ...
        {/* Timeline */}
        <div>
          <h3 className="font-semibold text-[#3B4D68] uppercase text-sm mb-4">TIMELINE</h3>
              ...
              {timelineSteps.map((step) => (
                <div key={step.id} className="flex gap-4 relative z-10 font-sans">
                  ...
                  <div className="pt-0.5">
                    <h4 className="font-medium text-gray-900 text-base">{step.label}</h4>
                    {step.date && (
                    ...
                    {step.id === 'accepted' && ticket?.status === 'not_start' && (
                      <span className="inline-block bg-blue-500 text-white text-[10px] font-medium px-3 py-1 rounded-full mt-2">Waiting</span>
                    )}
        ...
        {/* Upload Photo */}
        <div>
          <h3 className="font-semibold text-[#3B4D68] text-base mb-4">Upload Photo</h3>
